...
function setSessionCookie(res, token) {
    ...
    // Use SameSite=None for cross-origin with Secure, or Lax for development
    if (isProd) {
        parts.push('Secure');  // Only send over HTTPS in production
        parts.push('SameSite=None');  // Allow cross-origin with credentials
    } else {
        parts.push('SameSite=Lax');  // Less restrictive for development
    }
...
app.get('/api/admin/orders/:id/invoice', requireAdmin, (req, res) => {
    ...
    const invoiceUrl = `https://trusser.in/api/orders/${order.id}/invoice/pdf`;
